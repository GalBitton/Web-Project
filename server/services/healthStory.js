    ...
    createStory() {
        ...
        if (this.heartRate !== null) {
            if (this.heartRate < 60) {
                story.push("Your heart rate is on the lower side, indicating relaxation or possible bradycardia.");
            } else if (this.heartRate > 100) {
                story.push("Your elevated heart rate suggests stress, anxiety, or physical exertion.");
            } else {
                story.push("Your heart rate is within the normal range, reflecting a balanced state of health.");
            }
        }

        if (this.steps !== null) {
            if (this.steps < 5000) {
                story.push("Your step count suggests a sedentary lifestyle, which may impact your physical and mental health.");
            } else if (this.steps >= 10000) {
                story.push("Your step count suggests you're maintaining an active lifestyle, which is excellent for both physical and mental health.");
            } else {
                story.push("Your step count suggests you're moderately active, which is beneficial for your overall well-being.");
            }
        }

        if (this.sleepDuration !== null && this.sleepQuality !== null) {
            story.push(`You got ${this.sleepDuration.toFixed(2)} hours of sleep, with a quality rating of ${this.sleepQuality}.`);
            if (this.sleepQuality === "Very Poor" || this.sleepQuality === "Poor") {
                story.push("Poor sleep quality may lead to stress and reduced cognitive function.");
            } else if (this.sleepQuality === "Excellent") {
                story.push("Excellent sleep quality is a strong foundation for good mental health and daily performance.");
            }
        }

        if (this.stressScore !== null) {
            if (this.stressScore > 7) {
                story.push("Your high stress score indicates significant pressure. Consider relaxation techniques.");
            } else if (this.stressScore > 4) {
                story.push("You have a moderate level of stress, which is normal, but managing it is important for long-term health.");
            } else {
                story.push("Your stress levels are low, suggesting you're in a good mental state.");
            }
        }

        if (this.breathingRate !== null) {
            if (this.breathingRate < 12) {
                story.push("Your breathing rate is lower than usual, which might indicate relaxation or deep meditation.");
            } else if (this.breathingRate > 20) {
                story.push("A higher breathing rate suggests anxiety or physical exertion.");
            } else {
                story.push("Your breathing rate is within the normal range, reflecting a calm and balanced state.");
            }
        }
        ...
        if (this.bloodPressure && this.bloodPressure.systolic !== null && this.bloodPressure.diastolic !== null) {
            if (this.bloodPressure.systolic > 130 || this.bloodPressure.diastolic > 80) {
                story.push("Your blood pressure is elevated, which could be a sign of stress or cardiovascular strain.");
            } else {
                story.push("Your blood pressure is within a healthy range, supporting your overall well-being.");
            }
        }

        if (this.activityLevel !== null) {
            if (this.activityLevel > 75) {
                story.push("Your high activity level shows great physical fitness and resilience.");
            } else if (this.activityLevel < 30) {
                story.push("A low activity level might suggest fatigue or a sedentary lifestyle.");
            } else {
                story.push("Your activity level indicates a healthy balance of movement and rest.");
            }
        }

        if (this.focusScore !== null) {
            if (this.focusScore > 70) {
                story.push("Your focus is sharp, which is excellent for productivity and mental clarity.");
            } else if (this.focusScore < 30) {
                story.push("Your focus seems low, which might indicate mental fatigue or distraction.");
            }
        }
